const forecast = {
  "lat": 51.4589,
  "lon": -0.983,
  "timezone": "Europe/London",
  "timezone_offset": 3600,
  "hourly": [
    {
      "dt": 1633161600,
      "temp": 11.24,
      "feels_like": 10.61,
      "pressure": 1002,
      "humidity": 84,
      "dew_point": 8.64,
      "uvi": 0.19,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 7.03,
      "wind_deg": 173,
      "wind_gust": 12.57,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04d"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633165200,
      "temp": 11.1,
      "feels_like": 10.54,
      "pressure": 1002,
      "humidity": 87,
      "dew_point": 9.02,
      "uvi": 0.44,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 6.94,
      "wind_deg": 170,
      "wind_gust": 12.39,
      "weather": [
        {
          "id": 502,
          "main": "Rain",
          "description": "heavy intensity rain",
          "icon": "10d"
        }
      ],
      "pop": 0.23,
      "rain": {
        "1h": 4.21
      }
    },
    {
      "dt": 1633168800,
      "temp": 11.41,
      "feels_like": 10.85,
      "pressure": 1002,
      "humidity": 86,
      "dew_point": 9.15,
      "uvi": 0.5,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 7.82,
      "wind_deg": 167,
      "wind_gust": 13,
      "weather": [
        {
          "id": 502,
          "main": "Rain",
          "description": "heavy intensity rain",
          "icon": "10d"
        }
      ],
      "pop": 0.39,
      "rain": {
        "1h": 6.48
      }
    },
    {
      "dt": 1633172400,
      "temp": 12.19,
      "feels_like": 11.63,
      "pressure": 1001,
      "humidity": 83,
      "dew_point": 9.39,
      "uvi": 0.67,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 7.34,
      "wind_deg": 168,
      "wind_gust": 11.28,
      "weather": [
        {
          "id": 501,
          "main": "Rain",
          "description": "moderate rain",
          "icon": "10d"
        }
      ],
      "pop": 0.6,
      "rain": {
        "1h": 2.73
      }
    },
    {
      "dt": 1633176000,
      "temp": 11.74,
      "feels_like": 11.35,
      "pressure": 998,
      "humidity": 91,
      "dew_point": 10.32,
      "uvi": 0.75,
      "clouds": 100,
      "visibility": 9602,
      "wind_speed": 7.37,
      "wind_deg": 147,
      "wind_gust": 15.28,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10d"
        }
      ],
      "pop": 0.88,
      "rain": {
        "1h": 0.87
      }
    },
    {
      "dt": 1633179600,
      "temp": 12.64,
      "feels_like": 12.41,
      "pressure": 996,
      "humidity": 94,
      "dew_point": 11.7,
      "uvi": 0.21,
      "clouds": 100,
      "visibility": 9613,
      "wind_speed": 10.02,
      "wind_deg": 168,
      "wind_gust": 18.02,
      "weather": [
        {
          "id": 501,
          "main": "Rain",
          "description": "moderate rain",
          "icon": "10d"
        }
      ],
      "pop": 1,
      "rain": {
        "1h": 2.07
      }
    },
    {
      "dt": 1633183200,
      "temp": 13.62,
      "feels_like": 13.49,
      "pressure": 992,
      "humidity": 94,
      "dew_point": 12.39,
      "uvi": 0.16,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 10.3,
      "wind_deg": 168,
      "wind_gust": 17.57,
      "weather": [
        {
          "id": 501,
          "main": "Rain",
          "description": "moderate rain",
          "icon": "10d"
        }
      ],
      "pop": 1,
      "rain": {
        "1h": 1.13
      }
    },
    {
      "dt": 1633186800,
      "temp": 13.86,
      "feels_like": 13.73,
      "pressure": 990,
      "humidity": 93,
      "dew_point": 12.54,
      "uvi": 0.09,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 9.3,
      "wind_deg": 183,
      "wind_gust": 16.27,
      "weather": [
        {
          "id": 501,
          "main": "Rain",
          "description": "moderate rain",
          "icon": "10d"
        }
      ],
      "pop": 1,
      "rain": {
        "1h": 1.17
      }
    },
    {
      "dt": 1633190400,
      "temp": 13.71,
      "feels_like": 13.62,
      "pressure": 988,
      "humidity": 95,
      "dew_point": 12.78,
      "uvi": 0.04,
      "clouds": 100,
      "visibility": 7994,
      "wind_speed": 7.3,
      "wind_deg": 159,
      "wind_gust": 12.67,
      "weather": [
        {
          "id": 501,
          "main": "Rain",
          "description": "moderate rain",
          "icon": "10d"
        }
      ],
      "pop": 1,
      "rain": {
        "1h": 1.75
      }
    },
    {
      "dt": 1633194000,
      "temp": 13.25,
      "feels_like": 13.16,
      "pressure": 986,
      "humidity": 97,
      "dew_point": 12.64,
      "uvi": 0.01,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 4.05,
      "wind_deg": 123,
      "wind_gust": 9.08,
      "weather": [
        {
          "id": 501,
          "main": "Rain",
          "description": "moderate rain",
          "icon": "10d"
        }
      ],
      "pop": 1,
      "rain": {
        "1h": 1.62
      }
    },
    {
      "dt": 1633197600,
      "temp": 13.25,
      "feels_like": 13.19,
      "pressure": 985,
      "humidity": 98,
      "dew_point": 12.73,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 0.56,
      "wind_deg": 8,
      "wind_gust": 3.18,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10n"
        }
      ],
      "pop": 1,
      "rain": {
        "1h": 0.25
      }
    },
    {
      "dt": 1633201200,
      "temp": 13.21,
      "feels_like": 13.15,
      "pressure": 987,
      "humidity": 98,
      "dew_point": 12.6,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 4.16,
      "wind_deg": 258,
      "wind_gust": 11,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.26
    },
    {
      "dt": 1633204800,
      "temp": 13.15,
      "feels_like": 12.92,
      "pressure": 989,
      "humidity": 92,
      "dew_point": 11.64,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 6.54,
      "wind_deg": 258,
      "wind_gust": 12.8,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.18
    },
    {
      "dt": 1633208400,
      "temp": 11.49,
      "feels_like": 11.07,
      "pressure": 990,
      "humidity": 91,
      "dew_point": 9.83,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 4.98,
      "wind_deg": 241,
      "wind_gust": 11.95,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.18
    },
    {
      "dt": 1633212000,
      "temp": 11.35,
      "feels_like": 10.97,
      "pressure": 991,
      "humidity": 93,
      "dew_point": 9.96,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 6.61,
      "wind_deg": 234,
      "wind_gust": 13.42,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.14
    },
    {
      "dt": 1633215600,
      "temp": 11.51,
      "feels_like": 11.01,
      "pressure": 991,
      "humidity": 88,
      "dew_point": 9.32,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 5.61,
      "wind_deg": 244,
      "wind_gust": 11.63,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.1
    },
    {
      "dt": 1633219200,
      "temp": 11.14,
      "feels_like": 10.63,
      "pressure": 991,
      "humidity": 89,
      "dew_point": 9.16,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 4.42,
      "wind_deg": 239,
      "wind_gust": 9.96,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.07
    },
    {
      "dt": 1633222800,
      "temp": 10.63,
      "feels_like": 10.1,
      "pressure": 992,
      "humidity": 90,
      "dew_point": 8.81,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 4.1,
      "wind_deg": 241,
      "wind_gust": 9.14,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.11
    },
    {
      "dt": 1633226400,
      "temp": 10.31,
      "feels_like": 9.75,
      "pressure": 992,
      "humidity": 90,
      "dew_point": 8.59,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 3.84,
      "wind_deg": 241,
      "wind_gust": 7.7,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.15
    },
    {
      "dt": 1633230000,
      "temp": 9.98,
      "feels_like": 8.36,
      "pressure": 992,
      "humidity": 91,
      "dew_point": 8.32,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 3.22,
      "wind_deg": 246,
      "wind_gust": 7.07,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.11
    },
    {
      "dt": 1633233600,
      "temp": 9.61,
      "feels_like": 8.05,
      "pressure": 993,
      "humidity": 91,
      "dew_point": 7.9,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 2.99,
      "wind_deg": 237,
      "wind_gust": 6.04,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.15
    },
    {
      "dt": 1633237200,
      "temp": 9.43,
      "feels_like": 7.99,
      "pressure": 994,
      "humidity": 90,
      "dew_point": 7.71,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 2.73,
      "wind_deg": 227,
      "wind_gust": 5.95,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.15
    },
    {
      "dt": 1633240800,
      "temp": 9.14,
      "feels_like": 7.47,
      "pressure": 995,
      "humidity": 90,
      "dew_point": 7.34,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 3.01,
      "wind_deg": 229,
      "wind_gust": 7.21,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.11
    },
    {
      "dt": 1633244400,
      "temp": 9.54,
      "feels_like": 7.46,
      "pressure": 996,
      "humidity": 87,
      "dew_point": 7.39,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 3.95,
      "wind_deg": 224,
      "wind_gust": 9.1,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04d"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633248000,
      "temp": 10.58,
      "feels_like": 9.83,
      "pressure": 997,
      "humidity": 82,
      "dew_point": 7.33,
      "uvi": 0.34,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 4.73,
      "wind_deg": 222,
      "wind_gust": 9.43,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04d"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633251600,
      "temp": 11.76,
      "feels_like": 10.85,
      "pressure": 998,
      "humidity": 71,
      "dew_point": 6.62,
      "uvi": 0.82,
      "clouds": 98,
      "visibility": 10000,
      "wind_speed": 6.16,
      "wind_deg": 219,
      "wind_gust": 10.34,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04d"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633255200,
      "temp": 13.14,
      "feels_like": 12.18,
      "pressure": 998,
      "humidity": 64,
      "dew_point": 6.3,
      "uvi": 1.27,
      "clouds": 74,
      "visibility": 10000,
      "wind_speed": 8,
      "wind_deg": 223,
      "wind_gust": 12.05,
      "weather": [
        {
          "id": 803,
          "main": "Clouds",
          "description": "broken clouds",
          "icon": "04d"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633258800,
      "temp": 14.3,
      "feels_like": 13.38,
      "pressure": 999,
      "humidity": 61,
      "dew_point": 6.86,
      "uvi": 1.73,
      "clouds": 60,
      "visibility": 10000,
      "wind_speed": 9.2,
      "wind_deg": 225,
      "wind_gust": 13.55,
      "weather": [
        {
          "id": 803,
          "main": "Clouds",
          "description": "broken clouds",
          "icon": "04d"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633262400,
      "temp": 15.1,
      "feels_like": 14.23,
      "pressure": 999,
      "humidity": 60,
      "dew_point": 7.07,
      "uvi": 1.92,
      "clouds": 55,
      "visibility": 10000,
      "wind_speed": 9.69,
      "wind_deg": 231,
      "wind_gust": 13.86,
      "weather": [
        {
          "id": 803,
          "main": "Clouds",
          "description": "broken clouds",
          "icon": "04d"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633266000,
      "temp": 15.11,
      "feels_like": 14.35,
      "pressure": 1000,
      "humidity": 64,
      "dew_point": 8.22,
      "uvi": 2.07,
      "clouds": 35,
      "visibility": 10000,
      "wind_speed": 9.8,
      "wind_deg": 236,
      "wind_gust": 13.52,
      "weather": [
        {
          "id": 802,
          "main": "Clouds",
          "description": "scattered clouds",
          "icon": "03d"
        }
      ],
      "pop": 0.74
    },
    {
      "dt": 1633269600,
      "temp": 14.91,
      "feels_like": 14.23,
      "pressure": 1001,
      "humidity": 68,
      "dew_point": 8.69,
      "uvi": 1.57,
      "clouds": 38,
      "visibility": 10000,
      "wind_speed": 9.13,
      "wind_deg": 235,
      "wind_gust": 13.48,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10d"
        }
      ],
      "pop": 0.97,
      "rain": {
        "1h": 0.17
      }
    },
    {
      "dt": 1633273200,
      "temp": 14.71,
      "feels_like": 14.09,
      "pressure": 1002,
      "humidity": 71,
      "dew_point": 9.22,
      "uvi": 0.95,
      "clouds": 37,
      "visibility": 10000,
      "wind_speed": 7.83,
      "wind_deg": 240,
      "wind_gust": 12.74,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10d"
        }
      ],
      "pop": 0.98,
      "rain": {
        "1h": 0.13
      }
    },
    {
      "dt": 1633276800,
      "temp": 14.01,
      "feels_like": 13.4,
      "pressure": 1003,
      "humidity": 74,
      "dew_point": 9.32,
      "uvi": 0.42,
      "clouds": 38,
      "visibility": 10000,
      "wind_speed": 7.45,
      "wind_deg": 235,
      "wind_gust": 11.8,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10d"
        }
      ],
      "pop": 0.9,
      "rain": {
        "1h": 0.2
      }
    },
    {
      "dt": 1633280400,
      "temp": 12.91,
      "feels_like": 12.32,
      "pressure": 1004,
      "humidity": 79,
      "dew_point": 9.19,
      "uvi": 0.11,
      "clouds": 32,
      "visibility": 10000,
      "wind_speed": 6.73,
      "wind_deg": 239,
      "wind_gust": 11.91,
      "weather": [
        {
          "id": 802,
          "main": "Clouds",
          "description": "scattered clouds",
          "icon": "03d"
        }
      ],
      "pop": 0.76
    },
    {
      "dt": 1633284000,
      "temp": 11.71,
      "feels_like": 11.16,
      "pressure": 1005,
      "humidity": 85,
      "dew_point": 9.01,
      "uvi": 0,
      "clouds": 28,
      "visibility": 10000,
      "wind_speed": 5.85,
      "wind_deg": 240,
      "wind_gust": 11.95,
      "weather": [
        {
          "id": 802,
          "main": "Clouds",
          "description": "scattered clouds",
          "icon": "03n"
        }
      ],
      "pop": 0.58
    },
    {
      "dt": 1633287600,
      "temp": 10.91,
      "feels_like": 10.38,
      "pressure": 1006,
      "humidity": 89,
      "dew_point": 8.95,
      "uvi": 0,
      "clouds": 6,
      "visibility": 10000,
      "wind_speed": 4.74,
      "wind_deg": 228,
      "wind_gust": 10.76,
      "weather": [
        {
          "id": 800,
          "main": "Clear",
          "description": "clear sky",
          "icon": "01n"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633291200,
      "temp": 10.25,
      "feels_like": 9.73,
      "pressure": 1007,
      "humidity": 92,
      "dew_point": 8.77,
      "uvi": 0,
      "clouds": 5,
      "visibility": 10000,
      "wind_speed": 4.2,
      "wind_deg": 208,
      "wind_gust": 10.46,
      "weather": [
        {
          "id": 800,
          "main": "Clear",
          "description": "clear sky",
          "icon": "01n"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633294800,
      "temp": 10.05,
      "feels_like": 9.54,
      "pressure": 1007,
      "humidity": 93,
      "dew_point": 8.68,
      "uvi": 0,
      "clouds": 4,
      "visibility": 10000,
      "wind_speed": 4.44,
      "wind_deg": 199,
      "wind_gust": 11.27,
      "weather": [
        {
          "id": 800,
          "main": "Clear",
          "description": "clear sky",
          "icon": "01n"
        }
      ],
      "pop": 0
    },
    {
      "dt": 1633298400,
      "temp": 10.02,
      "feels_like": 9.53,
      "pressure": 1008,
      "humidity": 94,
      "dew_point": 8.82,
      "uvi": 0,
      "clouds": 3,
      "visibility": 10000,
      "wind_speed": 5.2,
      "wind_deg": 192,
      "wind_gust": 11.89,
      "weather": [
        {
          "id": 800,
          "main": "Clear",
          "description": "clear sky",
          "icon": "01n"
        }
      ],
      "pop": 0.01
    },
    {
      "dt": 1633302000,
      "temp": 10.63,
      "feels_like": 10.15,
      "pressure": 1008,
      "humidity": 92,
      "dew_point": 9.21,
      "uvi": 0,
      "clouds": 22,
      "visibility": 10000,
      "wind_speed": 5.96,
      "wind_deg": 198,
      "wind_gust": 13.22,
      "weather": [
        {
          "id": 801,
          "main": "Clouds",
          "description": "few clouds",
          "icon": "02n"
        }
      ],
      "pop": 0.01
    },
    {
      "dt": 1633305600,
      "temp": 11.04,
      "feels_like": 10.58,
      "pressure": 1008,
      "humidity": 91,
      "dew_point": 9.42,
      "uvi": 0,
      "clouds": 35,
      "visibility": 10000,
      "wind_speed": 6.11,
      "wind_deg": 202,
      "wind_gust": 13.29,
      "weather": [
        {
          "id": 802,
          "main": "Clouds",
          "description": "scattered clouds",
          "icon": "03n"
        }
      ],
      "pop": 0.02
    },
    {
      "dt": 1633309200,
      "temp": 11.19,
      "feels_like": 10.69,
      "pressure": 1008,
      "humidity": 89,
      "dew_point": 9.18,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 6.2,
      "wind_deg": 202,
      "wind_gust": 13.5,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10n"
        }
      ],
      "pop": 0.46,
      "rain": {
        "1h": 0.16
      }
    },
    {
      "dt": 1633312800,
      "temp": 11.73,
      "feels_like": 11.05,
      "pressure": 1008,
      "humidity": 80,
      "dew_point": 8.18,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 6.84,
      "wind_deg": 220,
      "wind_gust": 13.68,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10n"
        }
      ],
      "pop": 0.73,
      "rain": {
        "1h": 0.63
      }
    },
    {
      "dt": 1633316400,
      "temp": 11.51,
      "feels_like": 10.86,
      "pressure": 1008,
      "humidity": 82,
      "dew_point": 8.37,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 6.13,
      "wind_deg": 205,
      "wind_gust": 13.9,
      "weather": [
        {
          "id": 500,
          "main": "Rain",
          "description": "light rain",
          "icon": "10n"
        }
      ],
      "pop": 0.83,
      "rain": {
        "1h": 0.16
      }
    },
    {
      "dt": 1633320000,
      "temp": 11.69,
      "feels_like": 11.08,
      "pressure": 1008,
      "humidity": 83,
      "dew_point": 8.68,
      "uvi": 0,
      "clouds": 100,
      "visibility": 10000,
      "wind_speed": 6.67,
      "wind_deg": 205,
      "wind_gust": 14.05,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.86
    },
    {
      "dt": 1633323600,
      "temp": 11.14,
      "feels_like": 10.56,
      "pressure": 1009,
      "humidity": 86,
      "dew_point": 8.54,
      "uvi": 0,
      "clouds": 99,
      "visibility": 10000,
      "wind_speed": 5.74,
      "wind_deg": 211,
      "wind_gust": 12.51,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.82
    },
    {
      "dt": 1633327200,
      "temp": 10.17,
      "feels_like": 9.59,
      "pressure": 1009,
      "humidity": 90,
      "dew_point": 8.39,
      "uvi": 0,
      "clouds": 85,
      "visibility": 10000,
      "wind_speed": 4.85,
      "wind_deg": 213,
      "wind_gust": 11.51,
      "weather": [
        {
          "id": 804,
          "main": "Clouds",
          "description": "overcast clouds",
          "icon": "04n"
        }
      ],
      "pop": 0.74
    },
    {
      "dt": 1633330800,
      "temp": 10.1,
      "feels_like": 9.52,
      "pressure": 1010,
      "humidity": 90,
      "dew_point": 8.34,
      "uvi": 0,
      "clouds": 30,
      "visibility": 10000,
      "wind_speed": 4.59,
      "wind_deg": 211,
      "wind_gust": 11.16,
      "weather": [
        {
          "id": 802,
          "main": "Clouds",
          "description": "scattered clouds",
          "icon": "03d"
        }
      ],
      "pop": 0.02
    }
  ]
};

const sortForecast = () => {
  convertDate = (dt) => {
    const options = { hour: 'numeric', minute: 'numeric' };
    const currentHour = new Date(dt * 1000).toLocaleDateString("en-UK", options);

    return currentHour;
  }

  const sortFn = (a, b) => {

    // Snow
    if (a.snow && !b.snow) {
      return 1;
    } else if (b.snow && !a.snow) {
      return -1;
    } else if (a.snow && b.snow) {
      if (a.snow['1h'] === b.snow['1h']) {
        return b.feels_like - a.feels_like;
      }
      return a.snow['1h'] - b.snow['1h'];
    }

    // Rain
    if (a.rain && !b.rain) {
      return 1;
    } else if (b.rain && !a.rain) {
      return -1;
    } else if (a.rain && b.rain) {
      if (a.rain['1h'] === b.rain['1h']) {
        return b.feels_like - a.feels_like;
      }
      return a.rain['1h'] - b.rain['1h'];
    }

    // Temp
    return b.feels_like - a.feels_like;
  };

  forecast.hourly.forEach((hourly, i) => {
    forecast.hourly[i].dt = convertDate(forecast.hourly[i].dt);
  });

  forecast.hourly.sort(sortFn);

  var fs = require("fs");

  fs.writeFile("./forecast.json", JSON.stringify(forecast, null, 4), (err) => {
    if (err) { console.error(err); return; }
    console.log("File has been created");
  });
};


const sortDate = () => {
  const exampleDate = '1633348800';

  const today = new Date().toLocaleDateString();
  const todayEpoch = new Date(today).getTime()/1000;

  let tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate()+1);
  const tomorrowEpoch = new Date(tomorrow.toLocaleDateString()).getTime()/1000;

  console.log('exampleDate: ', exampleDate);
  console.log('todayEpoch: ', String(todayEpoch));
  console.log('tomorrowEpoch: ', String(tomorrowEpoch));
}

sortDate();
// sortForecast();